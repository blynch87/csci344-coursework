...
function getComments(comments) {
  if (comments.length > 1) {
    ...
    return ` 
        <button class="text-sm mb-3 text-blue-800">View all ${comments.length} comments</button>

        <p class="text-sm mb-3"><strong>
        ${lastComment.user.username}</strong> ${lastComment.text}
        </p>`;
  }

  if (comments.length === 1) {
    return `<p class="text-sm mb-3"><strong>
       ${comments[0].user.username}</strong> ${comments[0].text}
       </p>`;
  }
  return "";
}
